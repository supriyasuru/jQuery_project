$(document).ready(function(){
  		...
  		$(hvalue.sub).each(function(skey,svalue){
			...
			$(svalue.form).each(function(fkey,fvalue){
	  			if (fvalue.input == "radio"){
					...
					$( "main section:nth-child(" +(hkey+1)+ ") div:nth-child("+(skey+2)+")" ).append( '<p><label>'+fvalue.label+'</label> <'+fvalue.input+' id="'+fvalue.id+'" name="'+fvalue.name+'"  placeholder="'+fvalue.placeholder+'"></'+fvalue.input+'></p>');
	  			}
					...
					selec.appendTo('main section:nth-child('+(hkey+1)+' ) div:nth-child(' +(skey+2)+ ') ')
	  				...
	  				$( "main section:nth-child(" +(hkey+1)+ ") div:nth-child("+(skey+2)+")" ).append( '<p><label>'+fvalue.label+'</label> <input type="'+fvalue.input+'" id="'+fvalue.id+'" name="'+fvalue.name+'"  placeholder="'+fvalue.placeholder+'"></p>');
	  			}
  			});
  		});  		
  	});
	...
	$("#btn3").click(function(){ 
		...
		stor[head_select-1].sub[subhead_select-1].form.push({'input':'checkbox', 'label':label_value, 'id':inputid_value, 'name':name_value, 'value':value_value, 'placeholder':pholder_value, 'option':radioo_select});
			...
			var others_it=$('<p>')
			$('<label>'+label_value+'</label><input type="'+itype_select+'" id="'+inputid_value+'" name="'+name_value+'"  placeholder="'+pholder_value+'"></textarea>').appendTo(others_it)
